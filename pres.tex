...
\begin{frame}[t]
  ...
  Evaluating this on the entire domain is computationally intractable. 
  \emph{But}, we only care about the ODE ``fitting'' around the state estimate. So we can reduce our problem to matching along a finite number of points.

  \pause

  To get this state estimate we could use a spline smoothing approach:
